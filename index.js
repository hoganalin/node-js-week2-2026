...
function parseFileMetadata(file) {
  ...
  return {
    ...
    sizeKB: Math.round(file.size / 1024), //四捨五入
...
function sendJson(res, statusCode, body) {
  res.writeHead(statusCode, { "Content-Type": "application/json" }); //設定狀態碼和 header
  res.end(JSON.stringify(body)); //回傳json
}
...
function handleUpload(req, res, config) {
  ...
  form.on("error", (err) => {
    console.log("formidable error:", err.message); // 記錄 log、清理暫存檔、額外監控可以寫在這邊
  });
  form.parse(req, (err, fields, files) => {
    if (err) {
      return sendJson(res, 500, { error: err.message }); //錯誤回應
    }
    const uploadedFiles = files.file;
    if (!uploadedFiles) {
      return sendJson(res, 400, { error: "No file uploaded" }); //沒有檔案回應
    }
    const file = Array.isArray(uploadedFiles)
      ? uploadedFiles[0]
      : uploadedFiles; //如果是陣列取第一個
...
function router(req, res, config) {
  ...
  // 如果請求是 POST /coaches/avatar，呼叫 handleUpload
